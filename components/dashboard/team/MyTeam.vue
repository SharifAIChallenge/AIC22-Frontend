          ...
          <div class="team-card__header px-4">
            <div class="team-card__picture">
              <img
                ...
                :src="imageDisplay ? imageDisplay : this.team.image"
            ...
            <div v-if="!isEditing" class="team-card__edit">
              <v-icon right class="team-card__edit__icon" color="rgba(255, 255, 255, 0.12)" @click="toggleIsEditing">mdi-pencil</v-icon>
            </div>
          </div>
...
export default {
  ...
  methods: {
    onPictureChange(e) {
      ...
      this.imageDisplay = URL.createObjectURL(e);
      this.team.image = e;
    },
    leaveTeam() {
      this.loading = true;
      this.$axios.$delete('team').then(res => {
        ...
        if (res.status_code === 200 || res.status_code === 204) {
          ...
          this.toggleHaveTeam();
        } else this.$toast.error('خروج با مشکل مواجه شد!');
      });
    },
    updateTeamName() {
      this.loading = true;
